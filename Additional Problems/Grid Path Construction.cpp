...
string solve(int n, int m, int sy, int sx, int ty, int tx) {
    ...
    if (sx >= 2 && is_solvable(n, m - 2, sy, sx - 2, ty, tx - 2)) {
        ...
        ii cur = {sy, sx - 2};
        for (int i = 0; i < res.size(); ++i) {
            ii next = {cur.f1 + dy[P[res[i]]], cur.s2 + dx[P[res[i]]]};
            if (cur.s2 == 0 && next.s2 == 0) {
                return res.substr(0, i) + "L" + solve(n, 2, cur.f1, 1, next.f1, 1) + "R" + res.substr(i + 1, res.size());
            }
            cur = next;
        }
    }
    if (tx <= m - 3 && is_solvable(n, m - 2, sy, sx, ty, tx)) {
        ...
        for (int i = 0; i < res.size(); ++i) {
            ii next = {cur.f1 + dy[P[res[i]]], cur.s2 + dx[P[res[i]]]};
            if (cur.s2 == m - 3 && next.s2 == m - 3) {
                return res.substr(0, i) + "R" + solve(n, 2, cur.f1, 0, next.f1, 0) + "L" + res.substr(i + 1, res.size());
            }
            cur = next;
        }
    }
...
int main()
{
	...
	while (TC--)
	{
		...
		sr--; sc--; tr--; tc--;
		if (is_solvable(n, m, sr, sc, tr, tc))
		{
			cout << "YES\n";
			cout << solve(n, m, sr, sc, tr, tc) << '\n';
		}
		else
		{
			cout << "NO\n";
